...
function getClutterActor(obj)
{
	return (obj.actor == undefined) ? obj : obj.actor;
}
...
({
	...
	updatePanel: function(panel, main, lastUpdateGeom)
	{
		...
		let type = 0;
		let xid = 0;
		let clutterActor = getClutterActor(panel);
...
({
		...
		else if (uuid == PRL_INJ_WINDOW_LIST)
		{
			try
			{
				...
				this.Ext._winListInj['_populateWindowList'] = injectToFunction(
					...
					function()
					{
						let clutterActor = getClutterActor(this);
						if (clutterActor.coherenceId == undefined)
							clutterActor.coherenceId = coherence.getNextId();
						let id = clutterActor.coherenceId;
						let winId = id.toString();
	...
	enable: function()
	{
		...
		let _workspacesViewInit =
			(WorkspacesView.WorkspacesView.prototype['_init'] == undefined) ?
			'_updateWorkspaces' : '_init';
		this._workViewInj[_workspacesViewInit] = injectToFunction(
